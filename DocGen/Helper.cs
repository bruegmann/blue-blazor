...
public class Helper
{
    private PagesData _pagesData = new PagesData();
    private string _pagesPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "BlueBlazor.Docs", "Pages");
    ...
    public async Task Run()
    {
        ...
        RazorTemplate = "@{ /* This file is generated by DocGen. Don't make direct changes. */ }\n" +
            "{imports}\n" +
            "<DocGenContent BasePath=\"{pageUrl}\" Section=\"@Section\"><SideContent></SideContent>\n<ChildContent>{content}</ChildContent>\n" +
            "<PropsContent>{props}</PropsContent>\n" +
            "<ExamplesContent>{examples}</ExamplesContent>\n</DocGenContent>\n" +
            "@code { [Parameter] public string? Section { get; set; } }";

        await GenerateServicePages();

        await GenerateComponentPages(componentsPath, storiesPath, _pagesPath, dataPath,
            componentNamespace, assemblyName, xmlDocumentationPath);
        await _pagesData.CreateJsonFile();
        ...
        await GeneratePage(readmePath, indexRazorPath);

        string llmsTxtPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "BlueBlazor.Docs", "wwwroot", "llms.txt");
        await GenerateLLmsTxt(llmsTxtPath, readmePath, componentsPath, storiesPath, componentNamespace, assemblyName, xmlDocumentationPath);
    }

    public async Task GenerateServicePages()
    {
        string servicesPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "BlueBlazor", "Services");
        string storiesPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "BlueBlazor.Docs", "Stories");

        string xmlDocumentationPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "BlueBlazor", "bin", "Debug", "net10.0", "BlueBlazor.xml");

        string servicesNamespace = "BlueBlazor.Services";
        string assemblyName = "BlueBlazor";

        var services = Directory.GetFiles(servicesPath, "*Service.cs", SearchOption.AllDirectories)
            .Select(path => new
            {
        ...
        {
            var pagePath = Path.Combine(_pagesPath, $"{service.Name}.razor");

            string readMeHtml = MarkdownToHtml($"# {PascalToReadable(service.Name)}");
            string parametersHtml = "";

            Type? componentType = Type.GetType($"{servicesNamespace}.{service.Name}");
            if (componentType == null)
            {
                componentType = Type.GetType($"{servicesNamespace}.{service.Name}, {assemblyName}");
            }
            ...
            string pageUrl = $"servies/{PascalToKebabCase(service.Name)}";

            string pagesSectionName = PascalToReadable(service.ParentDirectory);
            string pagesComponentName = PascalToReadable(service.Name);

            if (_pagesData.ServicePageUrls.ContainsKey(pagesSectionName))
            {
                _pagesData.ServicePageUrls[pagesSectionName].Add(pagesComponentName, pageUrl);
            }
            else
            {
                _pagesData.ServicePageUrls.Add(pagesSectionName, new() { { pagesComponentName, pageUrl } });
            }

            string storyPath = Path.Combine(storiesPath, service.Name);
            string storyImport = "";
            string storyHtml = "";
            string sideContent = "";
            ...
            {
                storyImport = $"@using BlueBlazor.Docs.Stories.{service.Name}\n";
        ...
        string componentNamespace, string assemblyName, string? xmlDocumentationPath = null)
    {
        ...
        foreach (var component in components)
        {
            ...
            if (_pagesData.ComponentPageUrls.ContainsKey(pagesSectionName))
            {
                _pagesData.ComponentPageUrls[pagesSectionName].Add(pagesComponentName, pageUrl);
            }
            else
            {
                _pagesData.ComponentPageUrls.Add(pagesSectionName, new() { { pagesComponentName, pageUrl } });
            }
    ...
    private IEnumerable<ComponentParameterInfo> GetComponentParameters(Type componentType, string xmlDocumentationPath)
    {
        ...
        var retVal = componentType.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
